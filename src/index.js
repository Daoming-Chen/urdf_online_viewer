...
const updateAngles = () => {
    ...
    // Simple animation for UR5 robot
    const time = Date.now() / 1000;
    
    // Animate some joints with sine waves
    viewer.setJointValue('shoulder_pan_joint', Math.sin(time * 0.5) * 0.5);
    viewer.setJointValue('shoulder_lift_joint', Math.sin(time * 0.3) * 0.3 - 0.5);
    viewer.setJointValue('elbow_joint', Math.sin(time * 0.4) * 0.4);
    viewer.setJointValue('wrist_1_joint', Math.sin(time * 0.6) * 0.3);
    viewer.setJointValue('wrist_2_joint', Math.sin(time * 0.7) * 0.3);
    viewer.setJointValue('wrist_3_joint', time * 0.5);
